...
class QueryPlanningTraversal {
    constructor(parameters, selectionSet, startFetchIdGen, hasDefers, rootKind, costFunction, initialContext, typeConditionedFetching, excludedDestinations = [], excludedConditions = []) {
        var _a;
    ...
    resolveConditionPlan(edge, context, excludedDestinations, excludedConditions, extraConditions) {
        const bestPlan = new QueryPlanningTraversal({
            ...this.parameters,
            root: edge.head,
        }, extraConditions !== null && extraConditions !== void 0 ? extraConditions : edge.conditions, 0, false, 'query', this.costFunction, context, this.typeConditionedFetching, excludedDestinations, (0, query_graphs_1.addConditionExclusion)(excludedConditions, edge.conditions)).findBestPlan();
...
function computeRootParallelBestPlan(parameters, selection, startFetchIdGen, hasDefers) {
    const planningTraversal = new QueryPlanningTraversal(parameters, selection, startFetchIdGen, hasDefers, parameters.root.rootKind, defaultCostFunction, query_graphs_1.emptyContext, parameters.config.typeConditionedFetching);
                ...
                if ((0, query_graphs_1.isPathContext)(operation)) {
                    ...
                    if (conditions && edge.conditions) {
                        ...
                        if (contextToSelection) {
                            ...
                            updated.group.addAtPath(path.inGroup().concat(new federation_internals_1.Field(edge.head.type.typenameField())));
                        }
                        if (contextToSelection) {
...
function handleRequires(dependencyGraph, edge, requiresConditions, group, path, context, deferContext) {
    ...
    if (parents.length === 1 && pathHasOnlyFragments(path.inGroup())) {
        ...
        newGroup.addParents(createdGroups.map((group) => ({
            group,
            path: sameMergeAt(group.mergeAt, newGroup.mergeAt)
                && (0, federation_internals_1.sameType)(group.parentType, newGroup.parentType)
                ? []
                : undefined,
        })));
